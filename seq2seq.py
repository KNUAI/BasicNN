...
parser.add_argument('--bi', type=int, default=2, help='bidirectional')
...
class Encoder(nn.Module):
    ...
    def forward(self, src):
        ...
        output, (hidden, cell) = self.lstm(embedded)  #embedded  :  (src_len, batch_size, embed_size)
        if args.bi == 2:
...
class Decoder(nn.Module):
    ...
    def forward(self, input, hidden_set):

        #input = [batch_size]
        #hidden = [n_layers * n_directions, batch_size, hidden_size], where n_layers will always be 1
        #cell = [n_layers * n_directions, batch_size, hidden_size], where n_layers will always be 1
        ...
        enc_h, enc_c = hidden_set
        output, (hidden, cell) = self.lstm(embedded, (enc_h, enc_c))
        prediction = self.fc_out(output.squeeze(0))  #prediction  :  (batch_size, output_size)
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src, trg, teacher_forcing_ratio = 0.5):
        ...
        enc_output, enc_h, enc_c = self.encoder(src)  #encoder
        input = trg[0,:]  #<sos> tokens

        for t in range(1, trg_len):
            output, hidden, cell = self.decoder(input, (enc_h, enc_c))  #decoder
            outputs[t] = output
